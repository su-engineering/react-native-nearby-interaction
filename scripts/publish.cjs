...
async function main() {
  ...
  assert.equal(pkg.publishConfig.tag, distTag, 'npm tag must match the release channel');
  ...
  if (process.argv.includes('--dry-run')) {console.log(`Would publish ${pkg.name}@${pkg.version} with npm tag ${distTag} and create ${tag}`); return;}
  assert.ok(process.env.ACTIONS_ID_TOKEN_REQUEST_URL, 'Automated publication requires GitHub Actions trusted publishing');
  execFileSync('npm', ['publish', archive, '--access', 'public', '--tag', distTag, '--provenance', '--registry', 'https://registry.npmjs.org/'], {cwd: root, stdio: 'inherit'});
  ...
  if (release.status === 0) {
    ...
    execFileSync('gh', ['release', 'edit', tag, '--draft=false', `--prerelease=${prerelease}`, ...(prerelease ? [] : ['--latest']), '--notes-file', notes], {cwd: root, stdio: 'inherit'});
  } else {
    execFileSync('gh', ['release', 'create', tag, '--verify-tag', ...(prerelease ? ['--prerelease'] : ['--latest']), '--title', tag, '--notes-file', notes, archive, path.join(root, 'build/release/SHA256SUMS')], {cwd: root, stdio: 'inherit'});
  }
}
